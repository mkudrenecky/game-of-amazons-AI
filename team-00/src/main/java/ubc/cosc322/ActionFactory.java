...
public class ActionFactory {
            ...
            for (int j = 0; j < board.getBoardSize(); j++){
                if (board.getPieceAt(i,j) != 0 && board.getPieceAt(i, j) == player){
                    ...
                    for (QueenMove queenMove : queenMoves){
                        List<ArrowShot> arrowMoves = generateArrowMoves(board, queenMove.getEndRow(), queenMove.getEndCol(), queenMove.getStartRow(), queenMove.getStartCol());
    ...
    private static List<ArrowShot> generateArrowMoves(Board board, int startRow, int startCol, int queenStartRow, int queenStartCol){
